...
# Canonical room aliases (these map 1:1 to target)
ROOM_ALIASES_ZH = {
    "bathroom": ["廁所", "浴室", "洗手間"],
    "kitchen": ["廚房"],
...
# Extra room mentions for broader coverage (NOT part of label enum)
# These appear in raw_text but are normalized to allowed targets.
...
# Map extra room categories -> your allowed target enum
...
MEDIA_AUDIO_DEVICES = ["speaker", "soundbar"]  # treated as media power devices

def pick_room(weight_default: float = 0.25) -> str:
    """Pick a normalized target label room."""
...
def pick_room_word_and_target(base_target: str) -> Tuple[str, str]:
    """
    Returns (room_word, normalized_target).
    Sometimes returns canonical room word for base_target;
    sometimes returns an extra room word but normalized to allowed targets.
    """
    # 70%: canonical room alias from base_target
    if random.random() < 0.70:
        if random.random() < 0.5:
            return zh_room(base_target), base_target
        return en_room(base_target), base_target

    # 30%: extra room mention (normalize)
...
# Stable slot typing helpers (Arrow-friendly)
def _value_to_str(v):
    if v is None:
        return None
...
def emit_transcript(text, base_conf=0.15) -> Example:
    ...
    # transcripts use slots all null (keys present) by make_slots()
...
def gen_lights() -> Example:
    ...
    if style == "en":
        phr = random.choice([
            f"Turn on the {room_word} lights",
            f"Switch on the {room_word} light",
            f"Lights on in the {room_word}",
        ]) if onoff == "on" else random.choice([
            f"Turn off the {room_word} lights",
            f"Switch off the {room_word} light",
            f"Lights off in the {room_word}",
        ])
        return emit_command("lights", action, norm_target, onoff, make_slots(device="light"), phr, 0.92)

    if style == "mix":
        phr = random.choice([
            f"turn on {room_word}燈",
            f"Turn off {room_word}燈",
            f"{room_word} light on",
            f"{room_word} light off",
        ])
    ...
    # interpret: too bright => off; too dark/brighter => on
    if ("太亮" in phr) or ("too bright" in phr):
        return emit_command("lights", "turn_off", norm_target, "off", make_slots(device="light"), phr, 0.70)
    return emit_command("lights", "turn_on", norm_target, "on", make_slots(device="light"), phr, 0.70)
...
def gen_switches() -> Example:
    ...
    if style == "zh":
        ...
        phr = random.choice([
            f"幫我把{room_word}{name}打開",
            f"{room_word}{name}開一下",
            f"麻煩開{room_word}{name}",
        ]) if onoff == "on" else random.choice([
            f"幫我把{room_word}{name}關掉",
            f"{room_word}{name}關一下",
            f"麻煩關{room_word}{name}",
        ])
        return emit_command("switches", action, norm_target, onoff, make_slots(device=dev), phr, 0.86)

    if style == "en":
        phr = random.choice([
            f"Turn on the {dev} in the {room_word}",
            f"Switch on the {dev}",
            f"Please turn on the {dev} in the {room_word}",
        ]) if onoff == "on" else random.choice([
            f"Turn off the {dev} in the {room_word}",
            f"Switch off the {dev}",
            f"Please turn off the {dev} in the {room_word}",
        ])
        return emit_command("switches", action, norm_target, onoff, make_slots(device=dev), phr, 0.84)
    ...
    if style == "implicit":
        phr = random.choice([
            ...
            f"{room_word}有點臭",
            f"{room_word}太潮了",
            f"It's too humid in the {room_word}",
            f"It smells in the {room_word}",
            "空氣不太好",
        ])
        # best-effort mapping
        if ("潮" in phr) or ("humid" in phr):
            return emit_command("switches", "turn_on", norm_target, "on", make_slots(device="dehumidifier"), phr, 0.65)
        if ("悶" in phr):
            return emit_command("switches", "turn_on", norm_target, "on", make_slots(device="fan"), phr, 0.62)
        return emit_command("switches", "turn_on", norm_target, "on", make_slots(device="air_purifier"), phr, 0.62)

    # mix
    phr = random.choice([
        f"turn on {room_word} {dev}",
        f"turn off {room_word} {dev}",
    ])
...
def gen_climate() -> Example:
    ...
    if style == "zh_power":
        ...
        return emit_command("climate", "turn_on", norm_target, "on", make_slots(device="ac", mode="cool"), phr, 0.90)

    if style == "en_power":
        onoff = random.choice(["on", "off"])
        action = "turn_on" if onoff == "on" else "turn_off"
        phr = random.choice([
            f"Turn {onoff} the AC in the {room_word}",
            f"Switch {onoff} the air conditioner",
        ])
        return emit_command("climate", action, norm_target, onoff, make_slots(device="ac"), phr, 0.88)

    if style == "set":
        temp = random.choice([18, 20, 22, 24, 26, 28])
        phr = random.choice([
            f"{room_word}冷氣調到{temp}度",
            f"Set the {room_word} temperature to {temp}",
            f"把溫度設{temp}度",
        ])
        return emit_command(
            "climate", "set", norm_target, None,
            make_slots(device="thermostat", value=temp, unit="c", mode="setpoint"),
            phr, 0.86
        )
    ...
    if style == "mode":
        ...
        phr = random.choice([
            f"{room_word}冷氣切到{zh_mode}",
            f"Set AC mode to {mode}",
        ])
        return emit_command("climate", "set", norm_target, None, make_slots(device="ac", mode=mode), phr, 0.82)
    ...
    if style == "delta":
        ...
        return emit_command(
            "climate", "increase" if inc else "decrease", "default", None,
            make_slots(device="thermostat", value=delta, unit="c"),
            phr, 0.76
        )

    # implicit
    phr = random.choice([
        f"{room_word}好熱", f"{room_word}有點冷", "有點悶", "It's too hot", "I'm freezing",
    ])
    if ("熱" in phr) or ("too hot" in phr):
        return emit_command("climate", "turn_on", norm_target, "on", make_slots(device="ac", mode="cool"), phr, 0.62)
    if ("冷" in phr) or ("freezing" in phr):
        return emit_command("climate", "set", norm_target, None, make_slots(device="ac", mode="heat"), phr, 0.58)
    return emit_command("climate", "turn_on", norm_target, "on", make_slots(device="ac", mode="cool"), phr, 0.58)
...
def gen_media() -> Example:
    ...
    if style == "tv_power":
        onoff = random.choice(["on", "off"])
        phr = random.choice([
            f"Turn {onoff} the TV in the {room_word}",
            f"{'開' if onoff == 'on' else '關'}一下{room_word}電視",
        ])
        action = "turn_on" if onoff == "on" else "turn_off"
    ...
    if style == "volume":
        ...
        phr = random.choice([
            ...
            f"Turn it {'down' if direction == 'decrease' else 'up'}",
        ])
...
def gen_covers() -> Example:
    ...
    if style == "openclose":
        ...
        return emit_command("covers", oc, norm_target, None, make_slots(device=dev), phr, 0.88)

    if style == "position":
        pos = random.choice([10, 25, 50, 75, 90])
        phr = random.choice([
            f"Set the {room_word} {dev} to {pos}%",
            f"{room_word}窗簾開{pos}%",
            "窗簾開一半" if pos == 50 else f"窗簾開到{pos}%",
        ])
        return emit_command("covers", "set", norm_target, None, make_slots(device=dev, mode="position", value=pos, unit="percent"), phr, 0.84)
...
def gen_vacuum() -> Example:
    ...
    if style == "room":
        ...
        # set mode room with value being normalized target (stable enum)
...
def gen_timer() -> Example:
    ...
    if style == "set":
        ...
        phr = random.choice([
            f"幫我計時{minutes}分鐘",
            f"Set a {minutes} minute timer",
            f"在{room_word}設{minutes}分鐘計時",
        ])
        return emit_command("timer", "set", norm_target, None, make_slots(device="timer", value=minutes, unit="min", duration_sec=minutes * 60), phr, 0.90)
...
def gen_scene() -> Example:
    ...
    phr = random.choice([
        f"{scene} mode",
        f"切到{scene}模式",
        f"{room_word}開電影模式" if scene == "movie" else f"啟動{scene}模式",
        "Good night" if scene == "sleep" else "I'm leaving" if scene == "away" else f"Set {scene} scene",
    ])
    if phr == "Good night":
        scene = "sleep"
    if phr == "I'm leaving":
...
def gen_transcript() -> Example:
    texts = [
        ...
        "請幫我寄信給Jerry 謝謝",
        "你覺得怎麼樣",
        "turn on the light... uh actually never mind",
        "我剛剛說到哪",
        "哈哈哈",
        "幫我訂披薩",
        "你可以幫我查一下明天的行程嗎",
        ...
        "我想開心一點",
        "把我的心門打開",
        "Turn on your charm",  # looks like command but isn't smart-home
        "Can you open my mind?",
    ]
...
def generate(n: int, transcript_ratio: float = 0.30) -> List[Example]:
    # Weights: keep commands dominant but include negatives
    weights = {
        "lights": 0.14,
        "switches": 0.14,
        "climate": 0.12,
